...
public class PickHorizontalView extends RecyclerView {
    /**
    ...
    /**
     * 设置初始化时选中的位置,该方法必须在 setAdapter 之前调用
     *
    ...
    /**
     * 设置每次显示多少个item,该方法必须在 setAdapter 之前调用
     *
    ...
    public void onScrollStateChanged(int state) {
        ...
        if (state == SCROLL_STATE_IDLE) {
            ...
            ((IAutoLocateHorizontalView) wrapAdapter.adapter).onViewSelected(true, selectPos, null, itemWidth);
            if (listener != null) {
                listener.selectedPositionChanged(selectPos);
            }
        }
...
//                } else if(selectPos == position && position == (getItemCount() - 2)){
//                    ((IAutoLocateHorizontalView) adapter).onViewSelected(true, position, holder, itemWidth);
//                } else {
